...
export default class MediaEmbedIframely extends Plugin {
	...
	init() {
		...
		var customMediaConverterTimeout;
		//const iframelyInstance  = editor.config.get(( 'MediaEmbedIframely.iframely' ));
		...
		function _customMediaConverter( evt, data, conversionApi ) {
			clearTimeout(customMediaConverterTimeout);
			customMediaConverterTimeout = setTimeout( () => {
		...
		// editor.editing.downcastDispatcher.on( 'insert:media', ( evt, data, conversionApi ) => { setTimeout( () => {
		// 	const viewWriter = conversionApi.writer;
		// 	const viewWidget = conversionApi.mapper.toViewElement( data.item );
		// 	const Dom = editor.editing.view.domConverter.mapViewToDom( viewWidget );
		// 	const embedUrl =  data.item.getAttribute('url');
		//
		// 	console.log( 'data.item', data.item);
		// 	console.log( 'viewWidget', viewWidget);
		//
		// 	if (embedUrl!==undefined&&embedUrl!==null)
		// 	{
		// 		$.iframely.getPageData(embedUrl, function(error, data) {
		...
		// 			if ((error!==null&&error!==undefined)||data.links===undefined||data.links===null)
		// 			{
		// 				$(Dom).find('div.ck-media__wrapper').html(iframelyError);
		// 			}
		// 			else
		// 			{
		// 				var options = {
		...
		// 					});
		//
		// 					$(Dom).find('div.ck-media__wrapper').html($el);
		// 				}
		// 				else
		// 				{
		// 					$(Dom).find('div.ck-media__wrapper').html(iframelyError);
